...
impl CarettaId{
    ...
    /// # use caretta_id::*;
    /// 
    /// let base_time = NaiveDate::from_ymd_opt(2025, 1, 1).unwrap().and_hms_opt(0, 0, 0).unwrap().and_utc();
    /// let timestamp = NaiveDate::from_ymd_opt(2025, 11, 28).unwrap().and_hms_micro_opt(12,34,56,789).unwrap().and_utc();
    /// let id = CarettaId::from_timestamp_since(timestamp, base_time);
    /// assert_eq!(id.to_u64(), (((timestamp - base_time).num_milliseconds() / 100) as u64));
    /// ```
